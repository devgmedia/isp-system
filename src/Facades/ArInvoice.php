...
use App;
use App\Facades\ArInvoiceScheme;
use App\Facades\Calculate as CalculateFac;
use App\Facades\Mail as MailFac;
use App\Facades\MonthYear;
use App\Facades\Service;
use App\Facades\ZipStream as ZipStreamFac;
use App\Jobs\ArInvoiceCreateAndSendPdfReceiptWhatsapp;
use App\Jobs\ArInvoiceCreatePdf;
use App\Jobs\ArInvoiceCreatePdfReceipt;
use App\Jobs\ArInvoiceCreatePdfRetailInternet;
use App\Jobs\MidtransGetStatus;
use App\Mail\ArInvoice\Mail as Mailable;
use App\Mail\ArInvoice\ReceiptMail;
use App\Mail\ArInvoice\ReminderMail;
...
class ArInvoice
{
    ...
    public static function midtransHandle($order_id, $transaction_status, $fraud_status, $payment_type)
    {
        ...
        foreach ($invoice_midtrans as $invoice_midtrans_item) {
            ...
            if ($transaction_status === 'capture') {
                ...
                // bug
                // if ($invoice->payer_ref->device_token) {
                //     $log->save('create customer mobile apps notification');

                //     $title = 'Pembayaran Sukses';
                //     $body = 'Terima kasih telah melakukan pembayaran dengan nomor '.$invoice['number']
                //         .' sebesar '.$invoice['total'];
                ...
                //     $message = CloudMessage::withTarget('token', $invoice->payer_ref->device_token)->withNotification($notification);
    ...
    public static function createPdf(ArInvoiceModel $invoice, $disk = null)
    {
        ...
        if (!$disk) $disk = config('filesystems.primary_disk');
    ...
    public static function createPdfRetailInternet(ArInvoiceModel $invoice, $disk = null)
    {
        ...
        $payment_link = config('app.client_domain').'/pay'.'/'.$invoice_ref->uuid;
    ...
    public static function enterpriseCalculate(ArInvoiceModel $invoice_obj)
    {
        ...
        if ($invoice_customer_product->item_category) switch ($invoice_customer_product->item_category->name) {
            ...
            case 'Colocation':
                $tax_base_colocation += $invoice_customer_product->total;
                break;

            case 'Installation':
                $tax_base_installation += $invoice_customer_product->total;
                break;

            case 'Device':
                $tax_base_device += $invoice_customer_product->total;
                break;

            case 'Other':
                $tax_base_other += $invoice_customer_product->total;
                break;

            case 'Application':
                $tax_base_application += $invoice_customer_product->total;
                break;
        }
        ...
        $invoice_customer_product->invoice_customer_product_additionals->each(function ($invoice_customer_product_additional) use (
            ...
            if ($invoice_customer_product_additional->item_category) switch ($invoice_customer_product_additional->item_category->name) {
                ...
                case 'Colocation':
                    $tax_base_colocation += $invoice_customer_product_additional->total;
                    break;

                case 'Installation':
                    $tax_base_installation += $invoice_customer_product_additional->total;
                    break;

                case 'Device':
                    $tax_base_device += $invoice_customer_product_additional->total;
                    break;

                case 'Other':
                    $tax_base_other += $invoice_customer_product_additional->total;
                    break;

                case 'Application':
                    $tax_base_application += $invoice_customer_product_additional->total;
                    break;
            }
        });
        ...
        $invoice_obj->tax_base_colocation = $tax_base_colocation;
        $invoice_obj->tax_base_installation = $tax_base_installation;
        $invoice_obj->tax_base_device = $tax_base_device;
        $invoice_obj->tax_base_other = $tax_base_other;
        $invoice_obj->tax_base_application = $tax_base_application;
