...
use App\Facades\Customer as FacadesCustomer;
            ...
            DB::connection($conn)
                ...
                ->each(function ($service) use ($new_customer, $conn, $branch, $output, &$invoice_increment) {
                    ...
                    if ($service->aplikasi && $service->aplikasi > 0) {
                        $ar_invoice_item_category_id = ArInvoiceItemCategory::where('name', 'Application')->value('id');
                        $product_price = $service->aplikasi;
                    }

                    if ($service->lain2 && $service->lain2 > 0) {
                        $ar_invoice_item_category_id = ArInvoiceItemCategory::where('name', 'Other')->value('id');
                        $product_price = $service->lain2;
                    }

                    if ($service->perangkat && $service->perangkat > 0) {
                        $ar_invoice_item_category_id = ArInvoiceItemCategory::where('name', 'Device')->value('id');
                        $product_price = $service->perangkat;
                    }

                    if ($service->instalasi && $service->instalasi > 0) {
                        $ar_invoice_item_category_id = ArInvoiceItemCategory::where('name', 'Installation')->value('id');
                        $product_price = $service->instalasi;
                    }

                    if ($service->colocation && $service->colocation > 0) {
                        $ar_invoice_item_category_id = ArInvoiceItemCategory::where('name', 'Colocation')->value('id');
